...
def polygon_trap(uid, Omega, rf_voltages, dc_voltages, RFs, DCs, cover=(0, 0)):
    ...
    ycc = []
    zcc = []
    u_set = concatenate((array(rf_voltages), array(dc_voltages)))
    ...
    for iterr in range(nrf):
        ...
        yc = []
        zc = []
        polygon = array(RFs[iterr])
        no = array(polygon).shape[0]
        ...
        xcc.append(f'({u_set[iterr]/pi:e})*({xc})*cos(v_phase{uid}{iterr:d})')
        ycc.append(f'({u_set[iterr]/pi:e})*({yc})*cos(v_phase{uid}{iterr:d})')
        zcc.append(f'({u_set[iterr]/pi:e})*({zc})*cos(v_phase{uid}{iterr:d})')
    xcc = ' + '.join(xcc)
    ycc = ' + '.join(ycc)
    zcc = ' + '.join(zcc)    
    ...
    for ite in range(ndc):
        ...
        for m in range(2 * cover[0] + 1):
            ...
            for k in range(no):
                ...
                lt = (polygon[numt, 0] - polygon[numo, 0]) ** 2 + (polygon[numt, 1] - polygon[numo, 1]) ** 2
                if u_set[ite+nrf] != 0:
                    n = f'({u_set[ite+nrf]:e})*({rodc}+{rtdc})/({rodc}*{rtdc}*(({rodc}+{rtdc})*({rodc}+{rtdc})-({lt:e}))*{pi:e})'
...
def point_trap(uid, trap, cover=(0, 0)):
    ...
    for num, elec in enumerate(trap[0]):
        ...
        if elec[1] != 0:
            xrr.append(f'({elec[2][0]*elec[1]/2/pi:e})*({xr})*cos(v_phase{uid}{num:d})')
            yrr.append(f'({elec[2][0]*elec[1]/2/pi:e})*({yr})*cos(v_phase{uid}{num:d})')
            zrr.append(f'({elec[2][0]*elec[1]/2/pi:e})*({zr})*cos(v_phase{uid}{num:d})')
    ...
    for num, elec in enumerate(trap[1]):
        ...
        if elec[0] != 0:
            xcc.append(f'({elec[1][0]*elec[0]/2/pi:e})*({xc})')
            ycc.append(f'({elec[1][0]*elec[0]/2/pi:e})*({yc})')
            zcc.append(f'({elec[1][0]*elec[0]/2/pi:e})*({zc})')
...
def linear_shuttling_voltage(s, x0, d, T, dc_set, shuttlers=0, N=4, vmin=-15, vmax=15, res=25, L=1e-6, need_func=False, 
    ...
    """
    def q_tan(t):
        return array([d/2*(tanh(N*(2*t-T)/T)/tanh(N) + 1), 0, 0])    
...
def fitter_tan(t, a, b, c, d):
    return a*tanh(b*(t + c)) + d

def fitter_norm(t, a, b, c, d):
    return a*exp(b*(t-c)**2) + d
...
def approx_linear_shuttling(voltage_seq, T, res):
    ...
    """
    x_data = arange(res+1)*T/res
    funcs = []
    
    for i, seq in enumerate(voltage_seq):
        mean_seq = mean(seq)
        dif = seq[-1] - seq[0]
        ampl = max(seq)-min(seq)
        lin = std(seq)/max([mean_seq, mean(abs(seq)), 1e-6])
        att = 0
        if lin < 1e-4:
            funcs.append('(%5.3f)' % mean_seq)
        else:
            try:
                with catch_warnings():
                    simplefilter("ignore")
            ...
            try:
                with catch_warnings():
                    simplefilter("ignore")
                    popt_norm, _ = curve_fit(fitter_norm, x_data, seq, [abs(ampl), -1/2/T, T/2, min(seq)])
                    norm_seq = norm(seq - fitter_norm(x_data, *popt_norm))
            except:
                att +=1
                norm_seq = 1e6
            if att == 2:
...
def lossf_shuttle(uset, s, omegas, positions, L, dc_set, shuttlers, freq_coeff, freq_ax):
    ...
    '''
    rfss = array([0 for el in s if el.rf])
    for c, elec in enumerate(shuttlers):
        dc_set[elec] = uset[c]
    u_set = concatenate([rfss, array(dc_set)])
    loss = 0
    ...
    with s.with_voltages(dcs = u_set, rfs = None):
        if freq_coeff > 0:
            for i,x in enumerate(positions):
                try:
                    xreal = s.minimum(x*1.001, axis=(0, 1, 2), coord=identity(3), method="Newton-CG",
                    ...
                    xreal = x * 1.1
                    curv_z = omegas[i] * 1.1
                loss += sqrt(norm(xreal - x) ** 2 + freq_coeff*L**(-2)*norm(curv_z - omegas[i])**2)
        else:
            for i,x in enumerate(positions):
                try:
                    xreal = s.minimum(x*1.001, axis=(0, 1, 2), coord=identity(3), method="Newton-CG",
...
def shuttling_voltage(s, starts, routes, T, dc_set, shuttlers=0, vmin=-15, vmax=15, res=50, L=1e-6, need_func=False, 
    ...
    for i,start in enumerate(starts):
        with s.with_voltages(dcs = u_set, rfs = None):
            x1 = s.minimum(array(start), axis=(0, 1, 2), coord=identity(3), method="Newton-CG")
            curv_z, mod_dir=s.modes(x1,sorted=False)
            curvv = []
            for ax in freq_ax:
                curvv.append(curv_z[ax])
            curves.append(array(curvv))
            
    uset = zeros(len(shuttlers))
    for c, elec in enumerate(shuttlers):
        uset[c] = dc_set[elec]
    x = zeros((wells, 3))
    ...
    voltage_seq = array(voltage_seq).T
    volt_seq = []
    k = 0
    for i, v in enumerate(dc_set):
        if i in shuttlers:
            ...
            volt_seq.append(array([v]*(res+1)))

    volt_seq = array(volt_seq)
...
def polygon_shuttling(uid, Omega, rf_set, RFs, DCs, shuttlers, cover=(0, 0)):
    ...
    For example: V(t) = exp(-k*t**2) == "exp(-k*(step*dt)^2)",
    If const: V(t) = V == "V"
    ...
    for iterr in range(nrf):
        ...
        xcc.append(f'({rf_set[iterr]/pi:e})*({xc})*cos(v_phase{uid}{iterr:d})')
        ycc.append(f'({rf_set[iterr]/pi:e})*({yc})*cos(v_phase{uid}{iterr:d})')
        zcc.append(f'({rf_set[iterr]/pi:e})*({zc})*cos(v_phase{uid}{iterr:d})')
    ...
    for ite, elem in enumerate(DCs):
        ...
        xrr.append(f'({-1/pi:e})*({xr})*{shuttlers[ite]}')
        yrr.append(f'({-1/pi:e})*({yr})*{shuttlers[ite]}')
        zrr.append(f'({-1/pi:e})*({zr})*{shuttlers[ite]}')
    xrr = ' + '.join(xrr)
    yrr = ' + '.join(yrr)
    zrr = ' + '.join(zrr)
...
def n_wire_trap_design(top_dc, bottom_dc, central_wires, rf_indxs, Urf=0, gap=0, wire_splitting={}, L=1e-6, 
    ...
    Urf : float or array of floats shape([number of RF electrodes]), optional, default is 0
        Scaled RF amplitude of the trap. If scaled from SU rf amplitude (Vrf) as Urf = Vrf*sqrt(Z/mass)/(2*L*Omega), 
    ...
    try:
        ...
        Urf = ones(len(rf_indxs))*Urf

    for j, wire in enumerate(central_wires):
        central_wires[j] = array(wire) + array([gap, gap])
    wh_len_y = sum(central_wires, axis = 0)[1]
    ...
    if 0 in wire_splitting: 
        if 0 in rf_indxs:
            wire_splitting[0][1] = list(arange(len(wire_splitting[0][0])))
        ...
        if 0 in wire_splitting[0][1]:
            ...
            RF.append(array(celectrode_coordinates[-1])*L)
        else:
            st = f'dc[{count_dc}]'
            count_dc += 1
            electrodes.append([st, [celectrode_coordinates[-1]]])
            DC.append(array(celectrode_coordinates[-1])*L)
    ...
    for j, wire in enumerate(central_wires):
        if j > 0:
            if j in wire_splitting:
                if j in rf_indxs:
                    wire_splitting[j][1] = list(arange(len(wire_splitting[j][0])))
    ...
    electrode_coordinates = array(electrode_coordinates)*L
    DC = electrode_coordinates

    b_len = sum(bottom_dc, axis=0)[0]
    ...
    for j, elec in enumerate(bottom_dc):
        ...
        electrodes.append([st, [electrode_coordinates[-1]]])
    electrode_coordinates = array(electrode_coordinates)*L
    DC = concatenate([DC, electrode_coordinates])
    DC = concatenate([DCc, DC])
    ...
    s = System([PolygonPixelElectrode(cover_height=cheight, cover_nmax=cmax, name=n, paths=map(array, p))
                for n, p in electrodes_n])

    if len(Urf) != count_rf:
        warn('Provided RF voltages don\'t match RF electrodes')
    ...
    if need_plot:
        ...
        xmax = max(central_wires, axis = 0)[0]/2*1.2
        ymaxp = (max(top_dc, axis=0)[1] + wh_len_y/2) * 1.2
        ymaxn = (max(bottom_dc, axis=0)[1] + wh_len_y/2) * 1.2
...
def point_trap_design(frequencies, rf_voltages, dc_voltages, boundaries, scale, resolution, need_coordinates=False, 
    ...
    try:
        ...
        frequencies = ones(k)*frequencies
    ...
    for i, voltage in enumerate(rf_voltages):
        ...
        a_rf.append(areas)
        n = len(areas)
        voltages_rf.append(ones(n)*voltage)
        trap_rf.append([frequencies[i], voltage, areas, centers])
        omegas.append(ones(n)*frequencies[i])
        n_rf += n
    ...
    for i, voltage in enumerate(dc_voltages):
        ...
        n = len(areas)
        voltages_dc.append(ones(n)*voltage)
        n_dc += n
        
    x_rf = concatenate(x_rf)
    a_rf = concatenate(a_rf)
    voltages_rf = concatenate(voltages_rf)
    omegas = concatenate(omegas)
    if len(dc_voltages) > 0:
        x_dc = concatenate(x_dc)
        a_dc = concatenate(a_dc)
        x = concatenate((x_rf, x_dc))
        a = concatenate((a_rf, a_dc))
        voltages_dc = concatenate(voltages_dc)
        rf = concatenate((voltages_rf, zeros(n_dc)))
        dc = concatenate((zeros(n_rf), voltages_dc))
    else:
        x = x_rf
        a = a_rf
        rf = voltages_rf
...
def polygons_from_gds(gds_lib, L=1e-6, cheight=0, cmax=0, need_coordinates=True, need_plot=True, save_plot=None):
    ...
    for cell in lib.top_level():
        for coordinates in cell.get_polygons():
            ...
            count+=1
            full_elec.append(array(coordinates[::-1])*L)
    s = System([PolygonPixelElectrode(cover_height=cheight, cover_nmax=cmax, name=n, paths=map(array, p))
    ...
    if need_plot:
        ...
        for elec in full_elec:
            ymaxes.append(max(elec[:,1]/L))
            ymines.append(min(elec[:,1]/L))
            xmaxes.append(max(elec[:,0]/L))
            xmines.append(min(elec[:,0]/L))
...
def polygons_reshape(full_electrode_list, order, L=1e-6, need_plot=True, need_coordinates=True, 
    ...
    for i, elec in enumerate(order):
        ...
        full_elec.append(array(full_electrode_list[elec]))
    s = System([PolygonPixelElectrode(cover_height=cheight, cover_nmax=cmax, name=n, paths=map(array, p))
                for n, p in electrodes]) 
...
def gapping(elec, gap):
    ...
    for i, el in enumerate(newel):
        ...
        if nest[0]>0 and prev[1]<0:
            dot1 = el + array([gap, gap])
            dot2 = el - array([gap, gap])
...
def ioncloud_min(x, number, radius):
    ...
    for ind in range(number):
        ...
        positions.append([d * sin(a) * cos(b) + x[0],
                          d * sin(a) * sin(b) + x[1],
                          d * cos(a) + x[2]])
...
def ions_in_order(x, number, dist):
    ...
    '''
    positions = []
    xini = x - array([dist * number / 2, 0, 0])
    for ind in range(number):
        xi = xini + array([dist * ind, 0, 0])
        positions.append(xi)
...
def mathieu_solver(r, *a):
    ...
    * the eigenvalues and eigenvectors are not necessarily ordered
    ...
    """
    n = a[0].shape[0]
    m = zeros((2*r+1, 2*r+1, 2, 2, n, n), dtype=complex)
    for l in range(2*r+1):
        # derivative on the diagonal
        m[l, l, 0, 0] = m[l, l, 1, 1] = identity(n)*2j*(l-r)
        # the off-diagonal 1st-1st derivative link
        m[l, l, 0, 1] = identity(n)
...
def single_ion_modes(s, potential_minimum, ion_mass, L=1e-6, charge=1, mathieu=False, r=2, Omega=None):
    ...
    omegas = sqrt(charge * ct.e * curv_z / ion_mass) / (L * 2 * pi)

    if mathieu:
        if Omega is None:
            warn("Trap radio frequency must be provided for Mathieu modes calculation.")

        scale = sqrt(charge * ct.e / ion_mass) / (2 * L * Omega)
...
def equilibrium_ion_positions(s, dc_set, ion_masses, ion_number, potential_minimum=None, L=1e-6, 
    ...
    try:
        ...
        charges = ones(N)*charges
    try:
        ion_masses[0]
    except:
        ion_masses = ones(N)*ion_masses
    rf_set = s.rfs[s.rfs != 0]
    u_set = concatenate([zeros(len(rf_set)), dc_set])
    ...
    def system_potential(x):
        ...
        for i, x0 in enumerate(x):
            with s.with_voltages(dcs = u_set, rfs = rf_set*sqrt(ion_masses[0]*charges[i]/ion_masses[i]/charges[0])):
                pot += s.potential(x0)
                for j in range(i+1,N):
                    kap = ct.e*charges[i]*charges[j] / (4*pi*ct.epsilon_0)/L
    ...
    if positions_guess is None:
        with s.with_voltages(dcs = u_set, rfs = None):
            if potential_minimum is None:
                warn("Results may be wrong: potential_minimum guess is not provided.")
                potential_minimum = s.minimum(array([0, 1, 2]), method='Newton-CG')

            x_min = s.minimum(potential_minimum, method='Newton-CG')
            curv_z, mod_dir = s.modes(x_min, sorted=False)
            omega_sec=sqrt(charges[0]*ct.e*curv_z/ion_masses[0])/(L)
            l = (ct.e**2/(4*pi*ct.epsilon_0*ion_masses[0]*omega_sec[0]**2))**(1/3)/L
            positions_guess = zeros([N,3])

            for i in range(N):
                positions_guess[i] = x_min - array([l*(N-1)/2, 0, 0]) + array([i*l,0,0])

    positions_guess = concatenate(positions_guess)
    res = minimize(system_potential, positions_guess, method='L-BFGS-B', tol = 1e-16)
    equilibrium_positions = array(array_split(res.x, N))
...
def hessian(ion_positions, omega_sec, ion_masses, charges):
    ...
    '''
    ion_positions = array(ion_positions)
    omega_sec = array(omega_sec)*2*pi
    l = (ct.e**2 / (4*pi*ct.epsilon_0*ion_masses[0]*(omega_sec[0][2])**2))**(1/3)
    ion_positions = ion_positions/l
    N = len([len(a) for a in ion_positions])
    M_matrix = diag(list(array(ion_masses)**(-0.5))*3)

    def d(i, j):
        return norm(ion_positions[i] - ion_positions[j])
    a1 = []
    for i in range(N):
...
def normal_modes(ion_positions, omega_sec, ion_masses, charges = 1, linear = False, reshape = True):
    ...
    try:
        ...
        omega_sec = array([omega_sec for i in range(N)])
    A_matrix, M_matrix = hessian(ion_positions, omega_sec, ion_masses, charges)
    AA = dot(M_matrix, dot(A_matrix, M_matrix))

    eigen, norm_modes = eigh(AA)
    norm_modes = -norm_modes.T
    norm_freqs = sqrt(eigen)/(2*pi)
    if reshape:
    ...
    if linear:
        ...
        return array([x_freqs, y_freqs, z_freqs]), array([x_modes, y_modes, z_modes])
    else:    
...
def reshape_modes(ion_number, harm_modes, harm_freqs):
    ...
    for i in range(3*ion_number):
        ...
        if  k<1*ion_number:
            x_indx.append(i)
    indx = concatenate([x_indx, y_indx, z_indx])
...
def trap_hessian(ion_positions, s, rf_set, Omega, dc_set, ion_masses, charges, L, N, alpha, alpha2):
    ...
    for i,pos in enumerate(ion_positions):
        Urf = rf_set*sqrt(ct.e*charges[i]/ion_masses[i])/(2*L*Omega)

        with s.with_voltages(dcs = u_set, rfs = Urf):
            min_pos = array(s.minimum( 1.001*pos/L, axis=(0, 1, 2), coord=identity(3), method="Newton-CG"))
    ...
    A_hes = array(A_hes)
    Q_hes = array(Q_hes)
    real_hessian_A = zeros([3,3,N,N])
    real_hessian_Q = zeros([3,3,N,N])

    for p in range(3):
        for c in range(3):
            diag_A = array([hess[p,c] for hess in A_hes])
            real_hessian_A[p,c] = diag(diag_A)
            diag_Q = array([hess[p,c] for hess in Q_hes])
            real_hessian_Q[p,c] = diag(diag_Q)
            
    A = block([[real_hessian_A[0,0], real_hessian_A[0,1], real_hessian_A[0,2]],
                  [real_hessian_A[1,0], real_hessian_A[1,1], real_hessian_A[1,2]],
                  [real_hessian_A[2,0], real_hessian_A[2,1], real_hessian_A[2,2]]
                 ])
    Q = block([[real_hessian_Q[0,0], real_hessian_Q[0,1], real_hessian_Q[0,2]],
...
def crystal_modes(ion_positions, ion_masses, s, rf_set, Omega, dc_set, L = 1e-6, charges = 1, 
    ...
    norm_modes = norm_modes.T
    norm_freqs = mu[:3*N].imag*Omega/(2*pi)
...
def anharmonics(s, minimums, axis, L=1e-6):
    ...
    results = []
    for pos in minimums:
        x1 = s.minimum(array(pos), axis=(0, 1, 2), coord=identity(3), method="Newton-CG")
        ...
        k3 = eig(deriv3)[0][axis]*ct.e
        k4 = eig(deriv4)[0][axis]*ct.e
        l2 = (ct.e**2/(4*pi*ct.epsilon_0*k2))**(1/3)
        l3 = (k3/k2)**(1/(2-3))
        l4 = sign(k4)*(abs(k4)/k2)**(1/(2-4))
...
def stability(s, ion_masses, Omega, minimum, charges=1, L=1e-6, need_plot=True, save_plot=None):
    ...
    try:
        ...
        charges = ones(len(ion_masses)) * charges
    
    charges = array(charges) * ct.e
    params = {}
    for M, Z in zip(ion_masses, charges):
        rf_set = s.rfs
        rf_set = rf_set*sqrt(Z/M)/(2*L*Omega)
        scale = Z/((L*Omega)**2*M)
        with s.with_voltages(dcs = None, rfs = rf_set):
            x1 = s.minimum(array(minimum), axis=(0, 1, 2), coord=identity(3), method="Newton-CG")

            a = 4*scale*s.electrical_potential(x1, "dc", 2, expand=True)[0] 
            q = 4*sqrt(scale)*s.electrical_potential(x1, "rf", 2, expand=True)[0]      
            a, q = array(a), array(q)
            A = a
            Q = q
            A = A[1:3, 1:3]
    ...
    qa_crit_upper = [(-cc - sqrt(abs(cc**2 - 4*bb*(dd-aa))))/(2*(dd-aa)), (-cc + sqrt(abs(cc**2 - 4*bb*(dd-aa))))/(2*(dd-aa))]
    qa_crit_lower = [(-ff - sqrt(abs(ff**2 - 4*ee*(gg-hh))))/(2*(gg-hh)), (-ff + sqrt(abs(ff**2 - 4*ee*(gg-hh))))/(2*(gg-hh))]
...
def frequency_optimization(s, ion_masses, positions, axis, omegas, start_dcset, numbers=0, eps=1e-9, tol=1e-18,
    ...
    try:
        ...
        ion_masses = ones(positions.shape[0])*ion_masses
        M = ion_masses[0]
    try:
        Z = charges[0]
    except:
        charges = ones(positions.shape[0]) * charges
        Z = charges[0]
    ...
    def v_lossf(uset):
        with catch_warnings():
            ...
            pbarn.update(1)
            loss = 0
            u_set = zeros(len([rfs for rfs in s.rfs if rfs != 0]))
            u_set = concatenate([u_set, start_dcset])
            
            for i, pos in enumerate(positions):
                with s.with_voltages(dcs=u_set, rfs=rf_set*sqrt(M*charges[i]/ion_masses[i]/Z)):
                    try:
                        x1 = s.minimum(pos, axis=(0, 1, 2), coord=identity(3), method="Newton-CG",
                        ...
                        if any(isnan(omega_sec)):
                            loss += 100
                        else: 
                            loss += norm((omega_sec-omegas[i])/omegas[i][0])**2
                        
                        loss += micro*norm(x1-pos)**2
    ...
    if find_initial_guess:
        ...
        with tqdm(desc='Optimizing voltages', disable=True) as pbarn:
            uset = array(start_dcset)[[numbers]]
            initial_uset = uset[0]
...
def mode_optimization(s, alpha, potential_minimum, start_dcset, numbers=0, eps=1e-9, tol=1e-18, 
    ...
    def v_lossf(uset):
        with catch_warnings():
            ...
            with s.with_voltages(dcs=u_set, rfs=None):
                try:
                    x1 = s.minimum(potential_minimum*1.001, axis=(0, 1, 2), coord=identity(3), method="Newton-CG",
                                   tol = 1e-6, options = {'maxiter' : 2000})
                    _, mod_dir = s.modes(x1, sorted=False)
                    
                    e1 = array([0,0,1])
                    alp = arccos(inner(mod_dir[2],e1))
                    
                    loss += (alpha - alp)**2
                    loss += micro * norm(x1 - potential_minimum)**2
    ...
    def v_lossf(uset):
        with catch_warnings():
            ...
            for i, pos in enumerate(positions):
                with s.with_voltages(dcs=u_set, rfs=rf_set*sqrt(M*charges[i]/ion_masses[i]/Z)):
                    try:
                        x1 = s.minimum(pos*1.001, axis=(0, 1, 2), coord=identity(3), method="Newton-CG",
